        ...
        try (Scanner scanner = new Scanner(new File(nom_fichier))) {
            ...
            while (scanner.hasNext()) {
                 ...
                 * En informatique, la mémoïsation est la mise en cache des valeurs de retour d'une fonction selon ses valeurs d'entrée.
                 ...
                 J'étais sur la voie, mais quand même très loin de trouver toute seule :
                 ...
                 - if it starts with a #, check if it is long enough for the first group, check if all characters in the first [grouplength]
                    characters are not '.', and then remove the first [grouplength] chars and the first group number, and recursively check again.
                 - at some point you will get to the point of having an empty string and more groups to do - that is a zero,
                    or having an empty string with zero group to do - that is a one.
